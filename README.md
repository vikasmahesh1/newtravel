...
| Variable | Purpose |
| --- | --- |
| `VITE_API_BASE_URL` | Base URL for future API integrations. |
| `VITE_AUTH_TOKEN` | Example token or API key for authenticated requests. |

These values are not consumed by the mock services yet, but the structure is ready for live integrations (see [Roadmap](#roadmap)).
...
- **Mocked data services**: `services/mockApi.js` mimics asynchronous requests so the UI behaves like a real integration.
